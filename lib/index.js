...
const UPLOAD_FILE_RE = /^(up-[a-z0-9-]+)\.([a-z0-9]+)$/i;
/** Extensions treated as video wallpapers (all others → image). */
const VIDEO_EXTS = new Set(['mp4', 'webm', 'mov', 'mkv', 'avi']);
function isVideoExt(ext) { return VIDEO_EXTS.has(String(ext).toLowerCase()); }
    ...
    handler: (req, res) => {
      ...
      if (!ext) {
        ...
        res.end(JSON.stringify({
          error: '不支持的格式：' + ctype + '（支持 MP4 / WebM / MOV / MKV / AVI 视频与 JPG / PNG 图片）',
        }));
        return;
      }
      const chunks = [];
        ...
        try {
          ...
          if (dupId) {
            ...
            if (existing && existsSync(existing)) {
              ...
              const etype = isVideoExt(eext) ? 'video' : 'image';
