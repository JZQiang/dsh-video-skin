...
const DEFAULTS = {
  ...
  // "all" is the default here so locally-uploaded videos/images (which carry
  // no contentrating field) are always visible without extra clicks.
...
const UPLOAD_TYPES = ["image/jpeg", "image/png", "video/mp4", "video/webm", "video/quicktime", "video/x-matroska", "video/x-msvideo"];
...
async function uploadWallpaperFile(file) {
  ...
  if (!UPLOAD_TYPES.includes(ctype)) {
    selection.uploadError = "仅支持 MP4 / WebM / MOV / MKV / AVI 视频与 JPG / PNG 图片";
    emit();
    return;
  }
  if (!/\.(jpe?g|png|mp4|webm|mov|mkv|avi)$/i.test(file.name)) {
    selection.uploadError = "文件扩展名需为 .jpg / .png / .mp4 / .webm / .mov / .mkv / .avi";
    emit();
    return;
  }
...
function WallpaperPicker() {
  ...
  if (!sel.loaded) {
    return React.createElement("div", { className: "we-picker" },
      React.createElement("span", { className: "we-picker__hint" }, "加载壁纸…"));
  }
  if (sel.inventory.error) {
    return React.createElement("div", { className: "we-picker" },
      React.createElement("div", { className: "we-picker__error" },
        "壁纸服务不可用：" + sel.inventory.error),
  ...
  return React.createElement("div", { className: "we-picker", "data-we-cards": sel.pickerLayout },
    ...
    React.createElement("div", { className: "we-picker__card-head" },
      React.createElement("span", { className: "we-picker__card-name" }, "本地壁纸"),
      React.createElement("span", { className: "we-picker__card-badge" }, String(playableList.length)),
      React.createElement("span", { className: "we-picker__card-desc" }, "上传本地视频 / 图片当壁纸 · 液态玻璃主题"),
    ),
    // ── 上传本地壁纸: local JPG/PNG/MP4 as wallpapers. Files are written by the
    ...
    React.createElement("div", { className: "we-picker__section" },
      React.createElement("div", { className: "we-picker__section-head" },
        React.createElement("span", { className: "we-picker__section-label" }, "上传本地壁纸"),
      ),
      ...
      React.createElement("div", { className: "we-picker__row" },
        React.createElement("span", { className: "we-picker__hint we-picker__label" }, "选择文件"),
        React.createElement("input", {
          className: "we-picker__file", type: "file",
          accept: ".jpg,.jpeg,.png,.mp4,.webm,.mov,.mkv",
          disabled: sel.uploading,
          onChange: (e) => {
      ...
      React.createElement("div", { className: "we-picker__row" },
        ...
        React.createElement("span", { className: "we-picker__hint" }, "视频：MP4 / WebM / MOV / MKV · 图片：JPG / PNG"),
      ),
      uploadedList.length > 0 && React.createElement("div", { className: "we-picker__uploads-list" },
        uploadedList.map((w) => React.createElement("div", { key: w.id, className: "we-picker__uploads-item" },
          ...
          React.createElement("span", { className: "we-picker__hint" }, w.type === "video" ? "视频" : "图片"),
          React.createElement("button", {
            className: "we-picker__btn", type: "button",
            disabled: sel.uploading,
            onClick: () => {
              if (!window.confirm("移除壁纸「" + w.title + "」？此操作会删除本地文件，且不可恢复。")) return;
      ...
      React.createElement("div", { className: "we-picker__row" },
        ...
        React.createElement("span", { className: "we-picker__hint" }, "仅上传的壁纸"),
      ),
      ),
    ),
    ...
    editing && React.createElement("div", { className: "we-picker__editor" },
      ...
      React.createElement("div", { className: "we-picker__row" },
        ...
        React.createElement("option", { value: "" }, "从已有列表导入…"),
...
function apply(ctx) {
  ...
  if (ctx.slots) {
    ctx.slots.inject("settings.section", () =>
      ctx.slots.register(
        { name: "settings.section", id: "wallpaper-engine", order: 500, label: "本地壁纸" },
